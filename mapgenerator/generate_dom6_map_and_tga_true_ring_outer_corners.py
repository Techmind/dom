...
def draw_outer_corner_image_outside_ring(
    ...
    if not outer_corner_image_path or center is None or outer_radius is None:
        return
    if not os.path.exists(outer_corner_image_path):
        raise FileNotFoundError(f"Missing outer corner image: {outer_corner_image_path}")

    width, height = canvas.size
    cx, cy = center

    src = Image.open(outer_corner_image_path).convert("RGBA")
...
def draw_map_with_sprites(
    ...
    for p in provinces:
        ...
        if p.points:
            ...
            if p.role == "start":
                paste_tile_clipped_to_polygon(canvas, start_ring, p.points, (p.left, p.top, p.right, p.bottom))
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional image used to fill the empty square corners outside "
            "the outer circular ring, mirrored/flipped per quadrant."
        ),
    )
    parser.add_argument(
        "--ring-inner-radius",
